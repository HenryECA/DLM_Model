...
def main(hierarchy, series, results_path, train_ratio=0.8, s_matrix="summing"):
    ...
    for name, model in models.items():
        ...
        if len(model.data) > min_len:
            min_len = len(model.data)
    ...
    for h in range(30):
        for name in tree.all_nodes:
            ...
            horizon_residuals[h][name] = preds[:len(preds)-h] - models[name].test_unscaled[h:]
            ...
            for h in range(30):
                ...
                for node in tree.all_nodes:
                    ...
                    if reconciled_results is not None:
                        results[h][method][var][node] = {
                            'mu': list(reconciled_results[0][node].values),
                            'std': list(reconciled_results[1][node].values) if reconciled_results[1] is not None else None
                        }
    ...
    for h in range(30):
        for node in tree.all_nodes:
            ...
            base_test[h][node]['base']['base'] = {
                ...
                'mu': list(horizon_predictions[h][node].values),
                'std': list(horizon_variances[h][node].values) if horizon_variances[h][node] is not None else None
            }
